...
const InquiryModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 overflow-hidden">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-black/60 backdrop-blur-md cursor-pointer" onClick={onClose}></div>
      
      {/* Modal Container */}
      <div className="relative bg-white w-full max-w-md rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.3)] animate-in zoom-in duration-300 flex flex-col overflow-hidden border border-white/20 min-h-[400px]">
        
        {/* Decorative Green Bar */}
        <div className="absolute top-0 left-0 w-full h-1.5 bg-[#00a63e] z-30"></div>

        {status === "sent" ? (
          // SUCCESS UI
          <div className="p-10 flex flex-col items-center justify-center text-center animate-in fade-in zoom-in duration-500 h-full flex-1">
            ...
            <h3 className="text-2xl md:text-3xl font-black uppercase tracking-tighter mb-2 text-black">Inquiry Received</h3>
            <p className="text-zinc-500 font-medium text-sm max-w-sm mb-8">
              Form submitted successfully. Our team will get back to you shortly to discuss your project.
            </p>
            <button 
              onClick={onClose} 
          ...
          <>
            {/* Header */}
            <div className="p-7 pb-4 flex justify-between items-start bg-white shrink-0 z-20">
            ...
            {/* Compact Form Body */}
            <div className="px-7 py-2 flex-1 flex flex-col justify-center">
            ...
            {/* Premium Footer */}
            <div className="p-7 bg-white shrink-0 z-20">
              <button 
                ...
                className={`w-full py-5 rounded-[1.5rem] font-black uppercase tracking-[0.2em] text-[12px] transition-all flex items-center justify-center gap-3 active:scale-95 shadow-xl
                  ${status === "loading" ? "bg-black text-white opacity-80" : "bg-black text-white hover:bg-[#00a63e] hover:shadow-[#00a63e]/30"}`}
              >
                {status === "loading" ? "Validating..." : "Initialize Project"}
