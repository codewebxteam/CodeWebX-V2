...
const ImageUpload = ({ onUploadSuccess, folder = "general" }) => {
  ...
  return (
    <div className="group relative w-full h-52 bg-black border-2 border-dashed border-white/5 rounded-[2.5rem] flex flex-col items-center justify-center transition-all hover:border-[#00a63e]/30 overflow-hidden">
      {preview ? (
        <>
          <img src={preview} alt="Preview" className={`w-full h-full object-cover transition-all ${loading ? "opacity-20 blur-sm" : "opacity-40"}`} />
          ...
          <button 
            type="button"
            onClick={() => {setPreview(null); setUploadDone(false);}} 
        ...
        <label className="flex flex-col items-center justify-center cursor-pointer w-full h-full">
          ...
          <p className="text-[9px] font-black uppercase tracking-[0.3em] text-zinc-600">Select Media Source</p>
